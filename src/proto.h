...
#ifndef clink_proto_h
#define clink_proto_h
...
enum CorsairLinkCommands {
    ...
    /*
     * R - 2 bytes
     * Firmware Version in BCD (for example 1.0.5 is 0x1005, or 0x05, 0x10 in
     * little endianess)
     * */
    FirmwareID = 0x01,
    ...
    /*
     * RW - 2 bytes
     * In temperature controlled mode (0xC0) this defines the colour to use to
     * with the below gradients
     * */
    ...
    /*
     * RW - 2 bytes
     * Temperature limit (when the temperature goes over this, status is set to
     * 0xff)
     * */
    TEMP_Limit = 0x0F,
    ...
    /*
     * RW - 1 byte
     * Fan mode; 02=fixed PWM, 04=fixed RPM, 06=default, 08=quiet, 0a=balanced,
     * 0c=performance, 0e=custom; high bit is one when fan is detected,
     * low bit is one when the fan is 4-pin, bits 6~4 define the temperautre
     * channel to use in "curve" modes, 0 internal and 7 manual
     * */
    FAN_Mode = 0x12,
...
enum _CorsairLinkOpCodes {
    ...
    ReadTwoBytes = 0x09,
    /*
     * 0A AA 03 00 11 22 - Write 3-byte sequence (00 11 22) into 3-byte register
     * AA
     * */
